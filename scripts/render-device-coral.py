"""
Builds and renders the FORTH sterilizer — Coral colorway.
Based on render-device.py (midnight). Only the color palette and output paths differ.
Run: blender -b -P scripts/render-device-coral.py
Output: public/renderings/forth-device-coral-frame-00.png  (frame 0 only)

To render the full rotation later, set NUM_FRAMES = 104.
After rendering all frames, encode with ffmpeg:
  ffmpeg -r 30 -i "public/renderings/forth-device-coral-frame-%02d.png" ^
    -c:v libvpx-vp9 -g 1 -b:v 0 -crf 33 -an ^
    "public/renderings/forth-device-coral-rotation.webm"
"""
import bpy
import math
import os
...
# ── Coral palette (Blender linear RGB) ────────────────────────────────────────
BODY_DARK    = (0.052, 0.011, 0.011, 1.0)   # dark brownish-maroon
BODY_LIGHT   = (0.090, 0.022, 0.018, 1.0)   # slightly lighter
CAP_BASE     = (0.808, 0.330, 0.199, 1.0)   # #E89B7C coral glass
CAP_EDGE     = (0.950, 0.550, 0.380, 1.0)   # brighter coral edge accent
CAP_EMIT     = (0.808, 0.330, 0.199, 1.0)   # coral emission on facets
CAP_EDGE_EMIT= (0.950, 0.550, 0.380, 1.0)   # edge emission
EMITTER_COL  = (1.000, 0.850, 0.780, 1.0)   # warm-white inner glow
GLOW_COL     = (0.808, 0.330, 0.199, 1.0)   # compositor fog glow color
...
def configure_render():
    ...
    try:
        ...
        print(f"[render-device-coral] Compositor setup skipped: {e}")
