"""
Builds and renders the FORTH sterilizer — Sun colorway.
Based on render-device.py (midnight). Only the color palette and output paths differ.
Run: blender -b -P scripts/render-device-sun.py
Output: public/renderings/forth-device-sun-frame-00.png  (frame 0 only)

To render the full rotation later, set NUM_FRAMES = 104.
After rendering all frames, encode with ffmpeg:
  ffmpeg -r 30 -i "public/renderings/forth-device-sun-frame-%02d.png" ^
    -c:v libvpx-vp9 -g 1 -b:v 0 -crf 33 -an ^
    "public/renderings/forth-device-sun-rotation.webm"
"""
import bpy
import math
import os
...
CAP_EMIT     = (1.000, 0.797, 0.376, 1.0)   # golden emission on facets
CAP_EDGE_EMIT= (1.000, 0.920, 0.600, 1.0)   # edge emission
EMITTER_COL  = (1.000, 0.950, 0.800, 1.0)   # warm-white inner glow
GLOW_COL     = (0.909, 0.664, 0.128, 1.0)   # #F4D35E golden compositor glow
...
def configure_render():
    ...
    try:
        ...
        print(f"[render-device-sun] Compositor setup skipped: {e}")
...
def main():
    ...
    for i in range(NUM_FRAMES):
        ...
        frame_path = os.path.join(OUT_DIR, f"forth-device-sun-frame-{i:02d}.png")
        bpy.context.scene.render.filepath = frame_path
        bpy.ops.render.render(write_still=True)
        print(f"[sun] {i+1}/{NUM_FRAMES} → {frame_path}")

    shutil.copy(
        os.path.join(OUT_DIR, "forth-device-sun-frame-00.png"),
        os.path.join(OUT_DIR, "forth-device-sun-hero.png"),
    )
    device_root.rotation_euler = (0, 0, 0)
    print(f"[sun] Done. Frames in {OUT_DIR}")
