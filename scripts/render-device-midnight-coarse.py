"""
Midnight colorway — coarse matte texture variant.
Same scene as render-device.py but with larger voronoi cells, stronger bump,
and higher roughness for a more pronounced coarse matte surface.
Run: blender -b -P scripts/render-device-midnight-coarse.py
Output: public/renderings/forth-device-midnight-coarse-frame-00.png
"""
import bpy
import math
import os
...
def configure_render():
    ...
    try:
        ...
        print(f"[render-device-midnight-coarse] Compositor setup skipped: {e}")
...
def main():
    ...
    frame_path = os.path.join(OUT_DIR, "forth-device-midnight-coarse-frame-00.png")
    bpy.context.scene.render.filepath = frame_path
    bpy.ops.render.render(write_still=True)
    print(f"[midnight-coarse] Done → {frame_path}")
